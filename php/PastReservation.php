                ...
                <?php foreach ($reservations as $key => $reservation) { ?>
                    <div class="swiper-slide slide">
                        ...
                        <div class="content">
                            ...
                            <?php if ($reservation === $currentReservation) { ?>
                                <h3 style="font-size: 30px;">Your Current Reservation:</h3>
                                <a href="#reservation" class="btn">Modify Reservation</a>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            <div class="container">
                <?php if ($currentReservation) { ?>
                    <input type="hidden" name="reservation_id" value="<?php echo $currentReservation['id']; ?>">
                    <input type="hidden" name="action" value="modify">
                    <div class="box">
                        <p>Change Check In Date <span>*</span></p>
                        <input type="date" name="check_in" class="input" value="<?php echo $currentReservation['check_in']; ?>">
                    </div>
                    <div class="box">
                        <p>Change Check Out Date<span>*</span></p>
                        <input type="date" name="check_out" class="input" value="<?php echo $currentReservation['check_out']; ?>">
                    </div>
                    <div class="box">
                        <p>Change Time: <span>*</span></p>
                        <input type="time" name="time" value="<?php echo $currentReservation['time']; ?>">
                    </div>
                    <div class="box">
                        <p>Change Reservation Type <span>*</span></p>
                        <select name="type" class="input">
                            <option value="Meeting" <?php if ($currentReservation['type'] === 'Meeting') echo 'selected'; ?>>Meeting</option>
                            <option value="Exam" <?php if ($currentReservation['type'] === 'Exam') echo 'selected'; ?>>Exam</option>
                            <option value="Classroom" <?php if ($currentReservation['type'] === 'Classroom') echo 'selected'; ?>>Classroom</option>
                        </select>
                    </div>
                    <div class="box">
                        <p>More Notes to Add? <span>*</span></p>
                        <textarea name="notes" cols="100" rows="3" placeholder="Write your notes..." style="color: gray;"><?php echo $currentReservation['notes']; ?></textarea>
                    </div>
                ...
                <?php } else { ?>
                    <h1>No current reservation found.</h1>
                <?php } ?>
            </div>
        </form>
    </section>
